...
def main():
    ...
    # --- ensure each rank uses its own GPU ---
    import os, torch, torch.distributed as dist
    if args.launcher != 'none':
        # init_dist(...) already called above in your script
        local_rank = int(os.environ.get('LOCAL_RANK', os.environ.get('RANK', 0)))
        # If using legacy launcher, prefer --local_rank arg if present:
        if hasattr(args, 'local_rank') and args.local_rank is not None:
            local_rank = int(args.local_rank)
        torch.cuda.set_device(local_rank)
        if dist.is_initialized():
            print(f"[Rank {dist.get_rank()} | LocalRank {local_rank}] -> cuda:{torch.cuda.current_device()}", flush=True)
