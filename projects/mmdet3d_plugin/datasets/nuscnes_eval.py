...
def _subset_evalboxes(boxes, tokens):
    """Return a new EvalBoxes with only the given sample tokens."""
    try:
        # nuScenes devkit ≥ 1.1.10
        from nuscenes.eval.common.data_classes import EvalBoxes
    except ImportError:
        # older devkit (your other box likely used this)
    ...
    for tok in boxes.sample_tokens:
        if tok in keep:
            out.add_boxes(tok, boxes.boxes[tok])
    return out
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        :param verbose: Whether to print to stdout.
        """
        self.sample_tokens_override = sample_tokens
        self.nusc = nusc
        ...
        # Determine the tokens we will evaluate on.
        pred_tokens = set(self.pred_boxes.sample_tokens)
        gt_tokens   = set(self.gt_boxes.sample_tokens)

        if self.sample_tokens_override is not None:
            allowed = set(self.sample_tokens_override)
        else:
            # default to the official split tokens
            allowed = gt_tokens

        use_tokens = pred_tokens & gt_tokens & allowed

        missing_pred = allowed - pred_tokens
        missing_gt   = allowed - gt_tokens
        if missing_pred or missing_gt:
            print(f"[NuScenesEval_custom] Restricting to intersection. "
                f"use={len(use_tokens)}  missing_pred={len(missing_pred)}  missing_gt={len(missing_gt)}")

        # Filter both GT and predictions to the same token set.
        self.pred_boxes = _subset_evalboxes(self.pred_boxes, use_tokens)
        self.gt_boxes   = _subset_evalboxes(self.gt_boxes,   use_tokens)
        self.sample_tokens = sorted(list(use_tokens))

        # final sanity
        assert set(self.pred_boxes.sample_tokens) == set(self.gt_boxes.sample_tokens) == set(self.sample_tokens)
