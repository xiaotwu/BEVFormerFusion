...
class BevSumFusion(nn.Module):
    def __init__(self, cam_channels, lidar_channels, out_channels,
                 use_gate=True, init_gate=-2.0):
        super().__init__()
        self.proj_cam = nn.Identity() if cam_channels == out_channels else nn.Conv2d(cam_channels, out_channels, 1)
        self.proj_lidar = nn.Conv2d(lidar_channels, out_channels, 1)
        self.use_gate = use_gate
        if use_gate:
            # init_gate < 0 => start LiDAR contribution small
    ...
    def forward(self, bev_cam, bev_lidar):
        ...
        # Print once per process, only when fusion is actually used
        if self.use_gate and not hasattr(self, "_printed_gate_once"):
            print('[FUSION] gate =', torch.sigmoid(self.gate).item())
            self._printed_gate_once = True

        # resize lidar bev to match camera bev if needed
        if bev_lidar.shape[-2:] != bev_cam.shape[-2:]:
            bev_lidar = F.interpolate(
                bev_lidar, size=bev_cam.shape[-2:], mode='bilinear', align_corners=False
            )

        cam = self.proj_cam(bev_cam)
        lid = self.proj_lidar(bev_lidar)

        if self.use_gate:
            #alpha = torch.sigmoid(self.gate)  # scalar (0,1)
            alpha = 0.5
            return cam + alpha * lid
        else:
            return cam + lid
...
class BEVFormer(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        self.bev_fusion = BevSumFusion(
            cam_channels=256,     # BEVFormer camera BEV dim (often 256)
            lidar_channels=64,    # PillarFeatureNet feat_channels[-1]
            out_channels=256,
            use_gate=True,
            init_gate=-2.0
        )
